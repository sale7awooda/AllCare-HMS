...
// FIX: Added missing Recharts imports to resolve "Cannot find name" errors on charts
import { 
...
export const Billing = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const total = bills.reduce((acc, curr) => acc + (curr.paidAmount || 0), 0);
      const pending = bills.reduce((acc, curr) => acc + ((curr.totalAmount || 0) - (curr.paidAmount || 0)), 0);
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    if (patient && formattedItems.length > 0) {
      ...
      try {
        await api.createBill({ patientId: patient.id, patientName: patient.fullName, totalAmount: total, date: new Date().toISOString().split('T')[0], items: formattedItems });
    ...
    <div className="space-y-6">
            ...
            <Card title={t('billing_chart_revenue_by_type')}>
              <div className="h-80 w-full flex flex-col md:flex-row items-center gap-6">
                <div className="w-full md:w-1/2 h-full">
                  <ResponsiveContainer>
                    <PieChart>
                      <Pie data={stats.revenueByType} cx="50%" cy="50%" innerRadius={80} outerRadius={110} paddingAngle={5} dataKey="value" nameKey="name">
            ...
            <Card className="!p-0 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                  <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    <div className="relative w-full sm:w-64"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" /><input type="text" placeholder={t('billing_search_placeholder')} className="pl-9 pr-4 py-2 w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm" value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }}/></div>
                    <div className="relative w-full sm:w-auto"><Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4 pointer-events-none" /><select className="pl-9 pr-8 py-2 w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm appearance-none cursor-pointer" value={filterType} onChange={e => { setFilterType(e.target.value); setCurrentPage(1); }}><option value="all">{t('patients_filter_type_all')}</option><option value="Appointment">{t('nav_appointments')}</option><option value="Lab Test">{t('nav_laboratory')}</option><option value="Admission">{t('nav_admissions')}</option><option value="Operation">{t('nav_operations')}</option><option value="Procedure">{t('billing_type_procedure')}</option><option value="General">{t('billing_type_general')}</option></select></div>
                  </div>
                       ...
                       paginatedBills.map(bill => {
                        ...
                        return (
                          <tr key={bill.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm font-medium">{bill.patientName}</div><div className="text-xs text-slate-500">#{bill.patientId}</div></td>
      ...
      {activeTab === 'treasury' && (
          <div className="space-y-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="h-96 w-full bg-white dark:bg-slate-800 rounded-xl p-0 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden"><div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50"><h3 className="font-bold text-slate-800 dark:text-white">{t('billing_treasury_holdings')}</h3><p className="text-xs text-slate-500">{t('billing_treasury_holdings_subtitle')}</p></div><div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">{methodStats.map((item, idx) => { const Icon = getMethodIcon(item.name_en); return (<div key={idx} className="flex items-center gap-4 p-3 rounded-xl border border-slate-100 bg-slate-50"><div className={`p-3 rounded-full ${item.balance >= 0 ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600'}`}><Icon size={20} /></div><div className="flex-1 min-w-0"><div className="flex justify-between items-center mb-1"><h4 className="font-bold text-slate-700 truncate">{language === 'ar' ? item.name_ar : item.name_en}</h4><span className={`font-mono font-bold ${item.balance >= 0 ? 'text-slate-900' : 'text-red-500'}`}>${item.balance.toLocaleString()}</span></div><div className="flex items-center gap-3 text-xs"><span className="flex items-center gap-1 text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded"><ArrowUpRight size={10}/> ${item.income.toLocaleString()}</span><span className="flex items-center gap-1 text-red-600 bg-red-50 px-1.5 py-0.5 rounded"><ArrowDownRight size={10}/> ${item.expense.toLocaleString()}</span></div></div></div>);})}</div></div>
              </div>
              ...
              <Card className="!p-0 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                  <div className="p-4 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 flex flex-col md:flex-row gap-4 items-center">
                      ...
                      <div className="flex gap-2 items-center"><div className="flex items-center gap-1 text-xs text-slate-500"><span>{t('billing_treasury_from')}</span><input type="date" className="px-3 py-1.5 rounded-lg border border-slate-300 bg-white text-sm" value={treasuryDate.start} onChange={(e) => setTreasuryDate({...treasuryDate, start: e.target.value})}/></div><div className="flex items-center gap-1 text-xs text-slate-500"><span>{t('billing_treasury_to')}</span><input type="date" className="px-3 py-1.5 rounded-lg border border-slate-300 bg-white text-sm" value={treasuryDate.end} onChange={(e) => setTreasuryDate({...treasuryDate, end: e.target.value})}/></div><select className="px-3 py-1.5 rounded-lg border border-slate-300 bg-white text-sm" value={treasuryFilter} onChange={(e) => setTreasuryFilter(e.target.value)}><option value="all">{t('patients_filter_type_all')}</option><option value="income">{t('billing_treasury_type_income')}</option><option value="expense">{t('billing_treasury_type_expense')}</option></select></div>
                  </div>
                  <div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700"><thead className="bg-white dark:bg-slate-900"><tr><th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase">{t('date')}</th><th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase">{t('appointments_form_type')}</th><th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase">{t('billing_treasury_table_category')}</th><th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase">{t('billing_treasury_table_description')}</th><th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase">{t('billing_treasury_table_method')}</th><th className="px-6 py-3 text-right text-xs font-bold text-slate-500 uppercase">{t('billing_table_header_amount')}</th></tr></thead><tbody className="divide-y divide-slate-100 bg-white dark:bg-slate-800">{paginatedTransactions.map((tx) => (<tr key={tx.id}><td className="px-6 py-3 text-sm text-slate-600">{new Date(tx.date).toLocaleDateString()}</td><td className="px-6 py-3"><Badge color={tx.type === 'income' ? 'green' : 'red'}>{tx.type === 'income' ? t('billing_treasury_type_income') : t('billing_treasury_type_expense')}</Badge></td><td className="px-6 py-3 text-sm font-medium">{tx.category || '-'}</td><td className="px-6 py-3 text-sm text-slate-500">{tx.description}</td><td className="px-6 py-3 text-sm">{tx.method}</td><td className={`px-6 py-3 text-sm font-bold text-right ${tx.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>{tx.type === 'income' ? '+' : '-'}${tx.amount.toLocaleString()}</td></tr>))}</tbody></table></div>
                  <div className="px-6 py-3 border-t border-slate-200 flex justify-between items-center"><span className="text-sm text-slate-500">{t('patients_pagination_showing')} {treasuryPage} {t('patients_pagination_of')} {totalTreasuryPages || 1}</span><div className="flex gap-2"><button onClick={() => setTreasuryPage(p => Math.max(1, p - 1))} disabled={treasuryPage === 1} className="p-1 rounded hover:bg-slate-100 disabled:opacity-50"><ChevronLeft size={16}/></button><button onClick={() => setTreasuryPage(p => Math.min(totalTreasuryPages, p + 1))} disabled={treasuryPage === totalTreasuryPages} className="p-1 rounded hover:bg-slate-100 disabled:opacity-50"><ChevronRight size={16}/></button></div></div>
              </Card>
          </div>
      )}
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title={t('billing_modal_create_title')}>
        <form onSubmit={handleCreateSubmit} className="space-y-4">
          <Select label={t('billing_modal_create_select_patient')} value={createForm.patientId} onChange={e => setCreateForm({...createForm, patientId: e.target.value})} required><option value="">{t('billing_modal_create_choose_patient')}</option>{patients.map(p => <option key={p.id} value={p.id}>{p.fullName}</option>)}</Select>
          <div>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              {createForm.items.map((item, index) => (
                <div key={index} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center bg-slate-50 p-2 rounded-lg"><select className="w-full sm:w-1/3 text-xs p-2 rounded border border-slate-300" onChange={(e) => handleCatalogSelect(index, e.target.value)} defaultValue=""><option value="" disabled>{t('billing_modal_create_quick_add')}</option>{catalogItems.map((c, i) => <option key={i} value={c.label}>{c.label}</option>)}</select><Input placeholder={t('billing_modal_create_item_placeholder')} value={item.description} onChange={e => handleItemChange(index, 'description', e.target.value)} className="flex-1" /><div className="flex gap-2 w-full sm:w-auto"><Input placeholder="0.00" type="number" value={item.amount} onChange={e => handleItemChange(index, 'amount', e.target.value)} className="w-24" /><button type="button" onClick={() => handleRemoveItem(index)} className="text-red-500 p-2"><Trash2 size={16}/></button></div></div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 border-t pt-4"><div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">{t('billing_modal_create_tax_rate')}</label><select className="w-full rounded-xl border border-slate-300 p-2 text-sm" value={createForm.selectedTaxId} onChange={e => setCreateForm({...createForm, selectedTaxId: e.target.value})}><option value="">{t('billing_modal_create_none')}</option>{taxRates.filter(t => t.isActive).map(t => (<option key={t.id} value={t.id}>{language === 'ar' ? t.name_ar : t.name_en} ({t.rate}%)</option>))}</select></div><div className="text-right"><span className="block text-sm text-slate-500">{t('billing_modal_create_total_label')}</span><span className="text-2xl font-bold text-slate-900">${(() => { const subtotal = createForm.items.reduce((sum, i) => sum + (parseFloat(i.amount) || 0), 0); const taxRate = taxRates.find(t => t.id === parseInt(createForm.selectedTaxId)); const tax = taxRate ? (subtotal * taxRate.rate) / 100 : 0; return (subtotal + tax).toFixed(2); })()}</span></div></div>
          <div className="flex justify-end pt-4"><Button type="submit" disabled={isProcessing}>{isProcessing ? t('processing') : t('billing_modal_create_generate_button')}</Button></div>
        </form>
      </Modal>
      ...
      <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} title={t('billing_modal_payment_title')}>
        {payingBill && (
          <form onSubmit={handlePaymentSubmit} className="space-y-6">
            <div className="text-center p-4 bg-slate-50 rounded-xl"><p className="text-sm text-slate-500">{t('billing_modal_payment_balance_due')}</p><p className="text-4xl font-bold text-slate-900">${(payingBill.totalAmount - payingBill.paidAmount).toLocaleString()}</p></div>
            <Select label={t('billing_modal_payment_method')} value={paymentForm.method} onChange={e => setPaymentForm({...paymentForm, method: e.target.value})} required>{paymentMethods.filter(p => p.isActive).map(p => (<option key={p.id} value={p.name_en}>{language === 'ar' ? p.name_ar : p.name_en}</option>))}{!paymentMethods.some(p => p.name_en === 'Cash') && <option value="Cash">{t('billing_method_cash')}</option>}</Select>
