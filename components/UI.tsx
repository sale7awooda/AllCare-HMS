...
}> = ({ isOpen, onClose, onSubmit, title, itemDescription }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title}>
      <form onSubmit={handleSubmit} className="space-y-4">
        {itemDescription && (
          <div className="p-3 bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-200 rounded-lg text-sm border border-red-100 dark:border-red-900/30 font-medium">
            {itemDescription}
          </div>
        )}
        <Select label="Reason for Cancellation" value={reason} onChange={e => setReason(e.target.value)} required>
        ...
        <Textarea 
          ...
          placeholder="Provide more context about why this is being cancelled..."
          required
          rows={3}
        />
        <div className="flex justify-end gap-3 pt-2">
          <Button type="button" variant="secondary" onClick={onClose}>Keep</Button>
          <Button type="submit" variant="danger" disabled={!reason || !note}>Confirm Cancellation</Button>
        </div>
      </form>
    </Modal>
  );
