...
const getDbPath = () => {
  ...
  // Check if we are in a container with a /data volume (Railway persistent storage)
  if (fs.existsSync('/data')) {
      // Ensure we can write to /data
      try {
          fs.accessSync('/data', fs.constants.W_OK);
          return '/data/allcare.db';
      } catch (e) {
          console.warn('[Database] /data exists but is not writable. Falling back.');
      }
  }
  
  // Fallback to local directory (ephemeral in some deployments, but works for start)
...
if (!fs.existsSync(dbDir)) {
  try {
    ...
    console.error(`[Database] Failed to create directory ${dbDir}:`, e);
    // Ultimate fallback to temp dir if file system is strictly read-only
    const tmpPath = path.join(require('os').tmpdir(), 'allcare.db');
    console.log(`[Database] Switching to temp path: ${tmpPath}`);
    // We let it continue; if this fails, the try-catch below handles it.
  }
}
...
try {
  ...
  console.log("[Database] Attempting to start in-memory database for diagnostics...");
  try {
      db = new Database(':memory:');
      console.warn("[Database] Running in-memory mode. DATA WILL BE LOST ON RESTART.");
  } catch (innerE) {
      console.error("[Database] Failed to create in-memory database. Exiting.");
      process.exit(1);
  }
}
...
const initDB = (forceReset = false) => {
  ...
  try {
      ...
      console.error("[Database] Schema/Seed failed:", e);
  }
};
