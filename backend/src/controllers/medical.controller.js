...
exports.createLabRequest = (req, res) => {
  ...
  try {
    ...
    const createTx = db.transaction(() => {
      ...
      db.prepare('INSERT INTO billing_items (billing_id, description, amount) VALUES (?, ?, ?)').run(billId, `Lab Tests (Qty: ${testIds.length})`, totalCost);

      db.prepare(`
        INSERT INTO lab_requests (patient_id, test_ids, projected_cost, status, bill_id, created_at)
        VALUES (?, ?, ?, 'pending', ?, datetime('now'))
      `).run(patientId, JSON.stringify(testIds), totalCost, billId);
    });
  ...
  try {
    ...
    const processTx = db.transaction(() => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?)
      `).run(billId, `Surgery: ${op.operation_name}`, totalCost);

      db.prepare('UPDATE operations SET bill_id = ? WHERE id = ?').run(billId, id);
      ...
      if (details.participants && Array.isArray(details.participants)) {
        ...
        for (const p of details.participants) {
          if (p.staffId) {
            ...
            insertExtra.run(p.staffId, p.fee, reason, id);
          }
        }
      }
    });
...
exports.getInpatientDetails = (req, res) => {
  ...
  try {
    ...
    const getItems = db.prepare('SELECT description, amount FROM billing_items WHERE billing_id = ?');
    ...
    try {
        ...
        const generateTx = db.transaction(() => {
          ...
          if (totalPendingDebt <= 0.01) {
              throw new Error('PENDING_DEBT_ZERO');
          }
        ...
        if (err.message === 'PENDING_DEBT_ZERO') {
